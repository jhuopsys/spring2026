...
###########################################
DESIGN_DOC_LOC="src/vm/PART2_DESIGNDOC"
ASSIGNMENT_NAME="OS Assignment 3b"
CLASS="CS418/618"
GITHUB_ORG="jhuopsys"
...
expected_dir=$(git rev-parse --show-toplevel 2> /dev/null)
if [[ "${expected_dir}" != "${PWD}" ]]; then
    echo "${RED}Warning${NC}: You don't seem to be running this in a top-level directory"
    ...
    echo ""
    echo "Continue anyway? (type 'yes' to continue, anything else to exit)"
    read response
