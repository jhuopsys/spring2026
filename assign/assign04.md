...
**Assignment 4 is not required and does not count as extra credit**.
It is provided as a completely optional learning opportunity.
  </div>
</div>
...
You may build assignment 4 on top of assignment 2 or assignment 3. In either
case, all of the functionality needed for assignment 2 must work in your
filesys submission. If you build on assignment 3, then all of the assignment 3
functionality must work also, and you will need to edit
"`filesys/Make.vars`" to enable VM functionality.

<!--
 You can receive up to
10% extra credit if you do enable VM.
-->
...
"`filesys.c`"
:   Top-level interface to the file system. See section [4.1.2 Using the File System](assign02.html#b2-using-the-file-system), for an introduction.
...
assignment invoke Pintos a second time. The second run combines all the
...
assignment will not pass any of the extended file system tests until the
...
<span class='variable' markdown='1'>t</span> is named "<span class='variable' markdown='1'>t</span>`.tar`".
...
1.  Buffer cache (see section [3. Buffer Cache](assign04.html#3-buffer-cache)).
    Implement the buffer cache and integrate it into the existing file
    system. At this point all the tests from assignment 2 (and assignment 3,
    if you\'re building on it) should still pass.
2.  Extensible files (see section [1. Indexed and Extensible Files](assign04.html#1-indexed-and-extensible-files)). After this step, your assignment should
    pass the file growth tests.
3.  Subdirectories (see section [2.  Subdirectories](assign04.html#2-subdirectories)). Afterward, your assignment
...
Before you turn in your assignment, you must copy [the assignment 4 design
...
assignment. See section [D. Project Documentation](pintos/pintos_10.html#SEC153),
for a sample design document that goes along with a fictitious assignment.
...
<span class='variable' markdown='1'>n</span>-block file cannot be allocated even though <span class='variable' markdown='1'>n</span>
...
You can assume that the file system partition will not be larger than 8
...
<div class='admonition exercise'>
  <div class='title'>Exercise 4.2.1</div>
  <div class='content' markdown='1'>
Implement a hierarchical name space. In the basic file system, all files
live in a single directory. Modify this to allow directory entries to
point to files or to other directories.
...
<div class='admonition exercise'>
  <div class='title'>Exercise 4.2.2</div>
  <div class='content' markdown='1'>
...
directory separator character is forward slash ("<span class='sample' markdown='1'>/</span>"). You
must also support special file names "`.`" and "`..`", which have the
same meanings as they do in Unix.
...
(including "`.`" and "`..`") or create new files in a deleted directory
must be disallowed.

<div class='admonition exercise'>
  <div class='title'>Exercise 4.2.3</div>
  <div class='content' markdown='1'>
Implement the following new system calls:
  </div>
</div>

<u>System Call</u>: bool **chdir** (const char \*<span class='variable' markdown='1'>dir</span>)
:   Changes the current working directory of the process to
    <span class='variable' markdown='1'>dir</span>, which may be relative or absolute. Returns true if
    successful, false on failure.

<u>System Call</u>: bool **mkdir** (const char \*<span class='variable' markdown='1'>dir</span>)
:   Creates the directory named <span class='variable' markdown='1'>dir</span>, which may be relative
    or absolute. Returns true if successful, false on failure. Fails if
    <span class='variable' markdown='1'>dir</span> already exists or if any directory name in
    <span class='variable' markdown='1'>dir</span>, besides the last, does not already exist. That is,
    `mkdir("/a/b/c")` succeeds only if "`/a/b`" already exists and
    "`/a/b/c`" does not.

<u>System Call</u>: bool **readdir** (int <span class='variable' markdown='1'>fd</span>, char \*<span class='variable' markdown='1'>name</span>)

:   Reads a directory entry from file descriptor <span class='variable' markdown='1'>fd</span>, which
    must represent a directory. If successful, stores the
    null-terminated file name in <span class='variable' markdown='1'>name</span>, which must have room
...
<u>System Call</u>: bool **isdir** (int <span class='variable' markdown='1'>fd</span>)
:   Returns true if <span class='variable' markdown='1'>fd</span> represents a directory, false if it
    represents an ordinary file.

<u>System Call</u>: int **inumber** (int <span class='variable' markdown='1'>fd</span>)

:   Returns the *inode number* of the inode associated with
    <span class='variable' markdown='1'>fd</span>, which may represent an ordinary file or a
    directory.
...
The `pintos` "<span class='sample' markdown='1'>extract</span>" and "<span class='sample' markdown='1'>append</span>" commands
...
<div class='admonition exercise'>
  <div class='title'>Exercise 4.3</div>
  <div class='content' markdown='1'>
...
If you have `timer_sleep()` from the first assignment working, write-behind
...
in assignments that failed most or all of the tests.
  </div>
</div>
...
<div class='admonition exercise'>
  <div class='title'>Exercise 4.4</div>
  <div class='content' markdown='1'>
...
<div class='admonition caution'>
  <div class='title'>Note!</div>
  <div class='content' markdown='1'>
You can ignore this section: Assignment 4 is completely optional,
and we're not accepting submissions.
  </div>
</div>

The submission process is essentially the same as Assignments 1, 2, 3a, and 3b:

1. Download the submission script into the root of your development repository
2. Run the script
3. Enter the base commit (the hash of the last commit prior to starting work
  on this assignment)
4. Enter the names of the team members
5. Upload the resulting zipfile to Gradescope as "Assignment 4 (extra credit)"

Commands (where `$REPODIR` is the path to the root directory of your development
repository):

<div class='highlighter-rouge shell'><pre>
cd $REPODIR
curl -O https://jhuopsys.github.io/spring2025/assign/create_submission_a4.sh
chmod a+x create_submission_a4.sh
./create_submission_a4.sh
</pre></div>

Make sure you are switched to the branch containing your work before running
...
:   Here\'s a summary of our reference solution, produced by the
    ...
    enabled. See [the Assignment 3b FAQ](assign03b.html#faq), for the summary
    of project 3.
...
**How should a file name like "<span class='sample' markdown='1'>a//b</span>" be interpreted?**

:   Multiple consecutive slashes are equivalent to a single slash, so
    this file name is the same as "<span class='sample' markdown='1'>a/b</span>".

**How about a file name like "<span class='sample' markdown='1'>/../x</span>"?**

:   The root directory is its own parent, so it is equivalent to
    "<span class='sample' markdown='1'>/x</span>".

**How should a file name that ends in "<span class='sample' markdown='1'>/</span>" be treated?**
...
:   The goal of the 64-block limit is to bound the amount of cached file
    ...
    You can store a pointer to inode data in `struct inode`, but if you
    ...
    `byte_to_sector()` in "`filesys/inode.c`" uses the
    ...
    You will need to change `inode_byte_to_sector()` to obtain the
